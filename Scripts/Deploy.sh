...
# Retried, same as Deploy.ps1: Explorer/Defender/the indexer transiently hold a directory handle open
# right after a large exe is deleted from inside it often enough in practice that a single attempt
# flakes ("Device or resource busy"), and dying here throws away the whole publish that follows.
