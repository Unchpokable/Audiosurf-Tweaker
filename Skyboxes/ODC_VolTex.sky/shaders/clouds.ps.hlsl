// Pixel shader for this sky's cloud layer, compiled from here at run time. The manifest names the
// pair as "shaders/clouds"; this is the ".ps.hlsl" half, and it is where the clouds are *shaded*.
//
// Where they are *placed* is not here: the sprite positions, their clumping and the atlas they
// sample are built on the CPU from the layer's "prop" knobs (count, clumps, spread, scatter,
// elongation, floor, ceiling) - see Docs/Internal/skybox-geometry.md.
//
