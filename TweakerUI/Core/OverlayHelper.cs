...
{
    // Ported from SkinChangerRestyle.Core.OverlayHelper unchanged - no WPF dependency to begin with
    // (see revival roadmap Phase 4.3 notes). InjectHelper.exe/InternalOverlayRenderer.dll are the
    // frozen overlay components, still built on their old vcxproj outside the CMake/asbridge pipeline,
    // and still land in Plugins\ next to the app exe the same way they always did.
    internal class OverlayHelper
    {
        ...
        private readonly string _defaultD3DOverlayPlugin = "\\Plugins\\InternalOverlayRenderer.dll";
