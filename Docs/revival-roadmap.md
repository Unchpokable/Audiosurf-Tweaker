...
**Новый формат скинов** (в будущем `TweakerCore`, сейчас ещё `ChangerAPI`):
...
- `AudiosurfSkin.DeepClone()`/`AudiosurfSkinExtended.DeepClone()` переписываются на настоящий ручной глубокий копир объектного графа (сейчас это BinaryFormatter-трюк "сериализуй в память и обратно") — делать вместе с переписыванием модели, чтобы не тащить эту логику дважды.

**Новый формат цветовых пресетов** (`PaletteDynamicLoadContainer`, `ColorPalette`, `ColorPalettePrint`):
- Данные тривиальные (имя + 5 именованных цветов), zip-обёртка не нужна — просто `System.Text.Json` напрямую в `.pltc`/`.palette` файл.
- `PaletteDynamicLoadContainer.Load/Save` и `ColorPalette.Load/Save` переводятся на JSON-(де)сериализацию вместо `BinaryFormatter`.

**`LegacyDataConverter.exe` — отдельный проект, .NET Framework 4.8:**
- Один консольный инструмент на оба легаси-формата (не по конвертеру на каждый — см. раунд 4), компилируется против текущих (до-миграционных) типов `AudiosurfSkin`/`AudiosurfSkinExtended`/`ColorPalettePrint`/`PaletteDynamicLoadContainer`, которым для чтения всё ещё нужен `BinaryFormatter`.
- Вход: путь к файлу (+ тип определяется по расширению: `.tasp`/`.askin2` → скин, `.pltc`/`.palette` → пресет). Логика: прочитать старым способом (BinaryFormatter), записать рядом/на то же место в новом JSON/zip-формате. Отчитывается кодом возврата + сообщением в stdout.
- Основной Tweaker (уже на .NET 10) при попытке открыть файл, который не парсится в новом формате, ищет рядом `LegacyDataConverter.exe`, вызывает его как внешний процесс на файле, при успехе — перечитывает уже сконвертированный файл. Если конвертера нет или на машине нет .NET Framework — понятная ошибка пользователю, сам Tweaker не падает.
