...
- **Стили:** визуальная консистентность между модулями сделана отдельным проходом (см. Фазу 4.7 ниже) — акцентные + нейтральные цвета централизованы в словари ресурсов, а не просто «по ходу каждого модуля».
...
## Фаза 4.7 — Тёмная тема

**Статус: ✅ Выполнено, закоммичено и запушено (`b7e2443`, `3376dd6`).**

Сделано до Фазы 5, чтобы QuickPlayer с самого начала писался со ссылками на семантические токены темы, а не на хардкод.

- **Централизация цветов.** Новый `TweakerUI/Themes/NeutralColors.axaml` (Surfaces/Borders/Text — семантические токены, покрывают все 4 Styles-файла + Views) и переработанный `AccentColors.axaml` — оба переведены на Avalonia `ResourceDictionary.ThemeDictionaries` (`"Light"`/`"Dark"` варианты под одним ключом). Все `StaticResource` на цветовые ресурсы заменены на `DynamicResource`, чтобы UI перекрашивался вживую при смене темы без перезапуска.
- **Тёмная палитра** извлечена из макета пользователя через Claude Design MCP («Palette Designer - Dark»). Акцент меняет не только светлоту, но и оттенок (фиолетовый → бирюзово-голубой, `#2DD4BF`/`#22D3EE`/`#5EEAD4`). Найдены и заведены `AccentButtonGradientBrush` (CTA-кнопкам в тёмной теме нужен отдельный, более насыщенный градиент) и два новых нейтральных токена (`SurfaceBadgeColor`, `SurfaceElevatedColor`), которые в светлой теме были незаметно слиты с другими.
- **Живое переключение:** `SettingsProvider.IsDarkTheme` (персистится как обычная настройка), новый `TweakerUI/Core/ThemeService.cs` — единственная точка, которая меняет `Application.RequestedThemeVariant`; вызывается и при старте (после загрузки конфига), и из нового тумблера в Settings → Appearance.
- **Аудит и исправление анимаций кнопок по всему приложению.** Обнаружен и исправлен системный баг: дефолтный `ControlTheme` Avalonia для `Button` красит `:pointerover`/`:pressed` прямо на внутреннем `ContentPresenter` шаблона без единого перехода — это побеждает над любыми `Setter`-ами на самой кнопке, если кнопка не владеет собственным `ControlTemplate`. Тот же класс бага, что раньше был у `TextBox` (фокус-рамка) и `ListBoxItem` (цвет выделения). Исправлено централизованно: новый `TweakerUI/Themes/SharedControlTemplates.axaml` → `FlatButtonTemplate`, применён к 15 кнопочным классам по всем 4 Styles-файлам вместо копипасты шаблона в каждый класс. Корневая причина подтверждена сверкой с реальным исходником Avalonia (`gh api ...?ref=12.0.5`), а не по памяти — первая догадка (проблема в `SolidColorBrush.Opacity`) оказалась неверной.
- **Открытый вопрос:** остаётся ли `TWConsole` тёмной панелью всегда, независимо от темы приложения, или получает свой более тёмный Dark-вариант — не решено, токены под это уже готовы (`ConsoleBackgroundBrush`/`ConsoleTextBrush`).
