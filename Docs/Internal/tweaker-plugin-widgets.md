...
| `TweakerPlugin/src/ui/plugins/interactive/player.*` | Вкладка Quick Player (свой клиппованный список треков, не `list_view`) |
...
- **Иконка** (`set_icon("icons/play.svg")`) рисуется слева от метки, а без метки — по центру
  (иконочные кнопки транспорта Quick Player). Хранится **ключом ресурса**, а не разрешённым
  `image`: `ImTextureID` живёт только до смены устройства, поэтому `get_resource` зовётся на
  отрисовке (см. правило 2 в разделе про SVG). Размер выбирается по высоте кнопки: `>= 36px` → 24,
  иначе 16 — оба запечённые, так что ImGui не пересэмплирует текстуру. Тонируется цветом метки,
  то есть следует теме и press-анимации; иконка и метка центрируются как одна группа.
...
- `list_item_content::subtext` — опциональная вторая строчка под основной, помельче и приглушённая
  (имя плейлиста + количество треков). Пустая = однострочная строка, как у всех прежних вызывающих.
  `list_view` при этом поднимает высоту **всех** строк списка: список, где выше только часть строк,
  читается как сбитая раскладка, а не как информативная.
- Основной текст всегда обрезается **многоточием** (`detail::add_text_ellipsis`), а не клипом по
  краю строки: список длинных имён иначе заканчивается вертикальным срезом посреди глифа без всякого
  признака, что что-то потерялось.
...
- `set_selected(index)` — программный выбор, когда авторитетное значение живёт снаружи (вкладка
  Player подсвечивает тот плейлист, который реально отдал хост). Индекс вне диапазона снимает выбор;
  `selection_changed()` при этом **не** поднимается — он остаётся только про клик пользователя.
- **Нет клиппера.** Виджет создаёт по `list_item` на запись и каждый кадр гоняет для каждой
  `ButtonBehavior` + шаг tween'а. Для десятков строк это нормально, для тысячи — нет: список треков
  во вкладке Player поэтому нарисован вручную через `ImGuiListClipper`, а не этим виджетом.
...
Ряд взаимоисключающих чипов — оверлейный аналог `RadioButton.QPSegmentButton` из десктопного UI
(шесть режимов воспроизведения Quick Player, пара `Auto`/`Manual`).

```
┌───────────────────────────────────────────────┐
│ Single │ In order │ Repeat one │ [Shuffle]    │   ← выбранный чип в accent
└───────────────────────────────────────────────┘
```
...
static segmented modes{"qp_modes", 28.f};   // ctor принимает высоту, не ImVec2
static bool ready = false;
if (!ready) {
    const std::string_view labels[] = {"Single", "In order", "Shuffle"};
    modes.set_options(labels);              // тяжёлый: пересоздаёт анимации, не звать каждый кадр
    ready = true;
}

modes.set_selected(index_from_host);        // no-op если не изменилось; changed() НЕ поднимает
modes.update();

if (modes.hovered_index() >= 0) {           // тултип — забота вызывающего
    ImGui::SetTooltip("%.*s", (int)hint.size(), hint.data());
}
if (modes.changed()) {                      // только клик пользователя
    apply(modes.selected());
}
```

**Замечания:**

- **Shrink-to-content по ширине** (как `item_group`, в отличие от большинства виджетов): задаётся
  только высота. Чипы не сжимаются под панель — это панель должна расширяться (вкладка Player
  растягивает окно меню под свой контент).
- `width()` возвращает ту же ширину, которую посчитает `update()`. Shrink-to-content виджет иначе
  не разложить относительно чего-либо: транспортная строка Quick Player распределяет свободную
  ширину между группами, а для этого их ширины нужно знать **до** отрисовки.
- `set_icons(keys)` переводит полосу в **иконочный** режим: чипы становятся квадратными (16px глиф —
  запечённый размер, ImGui его не пересэмплирует), а метки уходят вызывающему на тултипы. Ключи
  хранятся, а не разрешаются: `ImTextureID` живёт только до смены устройства. Глиф тонируется цветом
  метки, то есть подхватывает crossfade выделения бесплатно.
- `changed()` поднимается **только** от клика. `set_selected` — нет, поэтому владелец может пушить
  авторитетное значение каждый кадр без петли против собственного клика пользователя.
- Hover анимируется только на невыбранных чипах: на выбранном он спорил бы с accent-заливкой и во
  время crossfade читался бы как два выбранных чипа сразу.
- Почему не циклическая иконка-кнопка: у состояний нет однозначных пиктограмм, а «включённое»
  состояние заливает кнопку акцентом и превращает иконку в сплошной квадрат. Проверено на десктопе
  на двух состояниях (Фаза 6.5) — слова читаются, иконка нет.

Smoke: группа **`Controls`** в [`smoke/main.cxx`](../../TweakerPlugin/smoke/main.cxx).

---

## `number_input`

Целочисленное поле с клампом и степперами `-`/`+`. Появилось под параметризованные теги песен Quick
Player (`[as-msz12]`, `[as-wb50]`, `[as-monopt3]` — 7 из 19), каждый из которых несёт свой диапазон
из каталога тегов.

```cpp
#include "ui/widgets/number_input.hxx"

static number_input match_size{"qp_msz", 96.f};
match_size.set_range(0, 24);        // клампит и текущее значение тоже
match_size.set_value(host_value);   // no-op если не изменилось; changed() НЕ поднимает
match_size.update();

if (match_size.changed()) {         // клик по степперу или уход из поля после ввода
    send(match_size.value());
}
```

**Замечания:**

- Построен на `ImGui::InputScalar`, перекрашенном через `theme` — тот же подход, что у `color_picker`
  с его RGBA-полями. Ввод текста, выделение и клавиатурное редактирование — слишком много поведения,
  чтобы переписывать его ради поля такого размера; совпасть должен был только chrome.
- Перекрашиваются и `ImGuiCol_Button*`, и `FrameRounding` — степперы `InputScalar` это **стоковые**
  кнопки ImGui, без этого они выходят серыми и прямоугольными рядом со всем остальным.
- `changed()` — **не** на каждое нажатие клавиши: два коммита, клик по степперу и
  `IsItemDeactivatedAfterEdit`. Иначе каждая набранная цифра была бы отдельным сообщением на проводе.

---

## `slider`

Дробный ползунок: метка и значение в одну строку, под ними тонкий трек с круглой ручкой. Появился
под параметры шейдерного неба (вкладка Skybox, `skybox/sky_panel.cxx`) — до него слайдера в
библиотеке не было вообще, и единственное место, которому он понадобился, брало стоковый
`ImGui::SliderFloat` и оставалось единственным raw-ImGui контролом во всём оверлее.

```cpp
#include "ui/widgets/slider.hxx"

static slider density{"c6x"};       // id из стабильного ключа параметра, не из метки
density.set_label("Star density");
density.set_range(0.005f, 0.30f);   // клампит и текущее значение
density.set_value(host_value);      // no-op если не изменилось; changed() НЕ поднимает
density.update();

if (density.changed()) {            // каждый кадр перетаскивания
    push_to_shader(density.value());
}
if (density.committed()) {          // отпустили мышь / приняли набранное число
    save_to_disk(density.value());
}
```

**Замечания:**

- **`changed()` и `committed()` — разные вещи, и это главное в API.** Перетаскивание — сотни
  значений, из которых на диск стоит писать одно. Скайбокс шлёт каждое промежуточное в живой шейдер
  (небо едет под мышкой) и переписывает файл настроек только на отпускании; см. аргумент `persist` у
  `skybox::set_program_param`.
- **Хват — вся строка, вместе с меткой.** В трек высотой 4px никто не должен целиться.
- **Shift во время drag** — точный режим (0.15 от обычной скорости). Не украшение: общая палитра
  объявляет `Light size` на диапазоне `0.99..1.0`, то есть весь ход укладывается в полтора пикселя
  двухсотпиксельного трека. Якорь перезакладывается на каждом переключении Shift — иначе вход в
  точный режим телепортирует ручку туда, куда её увёл бы грубый маппинг.
- **Двойной клик** превращает строку в `ImGui::InputFloat`, перекрашенный через `theme` (тот же
  приём, что у `number_input`). Вертикальный `FramePadding` обнуляется, чтобы поле было ровно в одну
  строку и высота строки не менялась между режимами — иначе панель прыгает на каждый двойной клик.
- **Точность вывода выводится из ширины диапазона**, а не фиксирована: `>= 100` → `%.0f`, `>= 10` →
  `%.1f`, `>= 1` → `%.2f`, иначе `%.4f`. Два знака превратили бы `Light size` в контрол, который
  показывает `1.00` на всём своём ходе, четыре — `Light glow` в `320.0000`.
- `set_value` игнорируется, пока идёт жест: владелец пушит авторитетное значение каждый кадр, и во
  время drag это значение пришло из самого виджета кадром раньше.

---

## `color_field`

Строка «метка + круглый свотч», по клику открывающая `color_picker` в собственном `popup_menu`.

Существует потому, что сам `color_picker` — большой виджет (SV-квадрат, полосы hue и alpha, hex и
четыре RGBA-поля), а одна только общая палитра неба объявляет четыре цвета: inline это четыре экрана
пикера на четыре значения, за свотчем — четыре строки. Вкладка Settings пришла к тому же выводу
раньше и собрала свотч-плюс-попап руками (`menu.cxx`, редактор темы); это та же композиция, ставшая
виджетом.

```cpp
#include "ui/widgets/color_field.hxx"

static color_field zenith{"c0xyz"};
zenith.set_label("Zenith");
zenith.set_color(host_color);   // игнорируется, пока попап открыт
zenith.update();

if (zenith.changed())   { push_to_shader(zenith.color()); }
if (zenith.committed()) { save_to_disk(zenith.color()); }
```

**Замечания:**

- Та же пара `changed()` / `committed()` и по той же причине, что у `slider`. `committed()`
  поднимается на кадре, когда пикер что-то менял, а кнопка мыши отпущена — тот же ленивый принцип,
  по которому меню сбрасывает конфиг только на кадрах без зажатой мыши.
- Открытие по **левому** клику. В редакторе темы это правый клик, но там свотч соседствует со
  списком, а здесь конфликта нет.
- Свечение свотча берёт **его собственный** оттенок, а не акцент темы: hover должен читаться как
  «этот цвет», а не как «здесь можно кликнуть».
- Пикер держится внутри и живёт столько же, сколько поле, — конструировать его на стеке нельзя
  (правило 1), у него есть и состояние HSV, и буферы полей ввода.
...
0. **`set_selected_tab(index)`** — программное переключение, анимируется ровно как клик по nav-кнопке
   и так же поднимает `selection_changed()` (переход настоящий в обоих случаях). Как и `set_tabs`,
   только снаружи `begin`/`end`; в debug это проверяется `assert`-ом. Используется смоуком, чтобы
   стартовать сразу на нужной вкладке.
...
4. **Ассеты монохромные**: белая заливка/обводка + альфа, `viewBox="0 0 24 24"`, только пути, без
   ...
   (альфа тоста).
5. **Слоты подбирать под запечённые размеры.** `pins` — строка 28px, слот 16px; `notefeed` —
   слот по высоте строки текста (`at(icon_px)`), строка тоста адаптивная: текст с переносом по
   ширине колонки, до 4 строк, плюс поля в пол-строки. Попадание в запечённый размер означает, что ImGui не пересэмплирует текстуру.
...
|---------|--------------|---------------------|
| Hover (button / list_item / tab / slider / свотч color_field) | ~150 ms | cubicOut |
| Press (button / tab) | ~90 ms | quadraticOut |
| Slider drag (ручка) | ~90 ms | quadraticOut |
...
### `detail::dt_ms()` и высокий FPS

`tweeny` шагает целыми миллисекундами, а кадр может быть короче миллисекунды. Прежняя реализация
`dt_ms()` клампила результат снизу единицей, то есть **выше ~1000 FPS каждый кадр заявлял, что прошла
целая миллисекунда**, хотя прошла её доля. Все анимации в этом режиме идут в `FPS/1000` раз быстрее:
на нескольких тысячах кадров, которые легко выдаёт окно без vsync, hover в 150 ms укладывается в
десятки миллисекунд, а press в 90 ms не виден вообще — выглядит ровно как «анимации выпилили».

Сейчас дробный остаток **переносится в следующий кадр**, поэтому суммарное время точное на любом FPS,
а `dt_ms()` может вернуть `0` — и это правильно, тогда tween просто не двигается в этом кадре.
Значение считается один раз за кадр и кэшируется по `ImGui::GetFrameCount()`: его дёргает каждый
анимированный виджет, и общий остаток, потребляемый на каждый вызов, достался бы первому обратившемуся.

`smoke_test` с тех пор включает vsync (`wglSwapIntervalEXT(1)`) — судить о тайминге анимаций по
частоте кадров, которой в игре не бывает, всё равно бессмысленно.
