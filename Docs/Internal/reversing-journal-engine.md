...
> **Уточнено `reversing-journal-boot.md` (22.09.2026).** Кадр устроен точнее, чем описано здесь:
> главный цикл `QuestViewer.exe` держит **`EngineControl`**, а не `EngineInterface`, и зовёт
> `EngineControl::EngineLoop()`, внутри которой ровно один `IncreaseTreeCount()` и ровно один
> `CallStartChannel()` — **только у стартовой группы**, а не «каждая группа получает». Остальные
> достигаются по ссылкам из неё. Там же: `EngineInterface*` доступен с первого кадра через
> `EngineControl::GetEngineInterface()`, то есть детур на `CallChannel` ради него не нужен вовсе.
> Полный перечень уточнений к этому журналу — §9 того документа.
...
`tw::lua::channels::get_vector`.

#### `SetVector` — как передаётся аргумент и что она делает сверх записи

Разобрана по дизассемблеру (RVA `0x1140` в той же DLL), потому что от обоих ответов зависит
корректность плагина.

**Передача аргумента.** `D3DXVECTOR3` идёт **по значению**, что видно уже из декорированного имени
(`?SetVector@…@@UAEXUD3DXVECTOR3@@@Z` — `U…`, а не `PAU…` или `AAU…`), и подтверждается прологом:
...
push ebx / push ebp
mov  ebp, ecx              ; this
mov  ecx, [esp+0x90]       ; x   (== исходный [esp+4])
lea  eax, [ebp+0x7c]       ; хранилище вектора канала
mov  [eax], ecx
mov  ecx, [esp+0x98]       ; z   (== исходный [esp+0xC])
mov  [eax+4], edx
mov  [eax+8], ecx
```

То есть три компонента читаются из трёх подряд идущих dword'ов начиная с `[esp+4]` — раскладка,
байт в байт совпадающая с тремя отдельными `float`-аргументами. Поэтому в плагине тип объявлен как
`void(__fastcall*)(A3d_Channel*, void* edx, float, float, float)` и никакого struct-типа не нужно.
Проверено офлайн-тестом (`shimtest`), где вызов через этот тип попадает в обычный `__thiscall`-метод,
принимающий 12-байтную структуру по значению.

Заодно видно, что вектор канала лежит на **`+0x7c`** — там же, где `channelFloat_` у числового
(§3.1).

**Побочный эффект, который важнее самой записи.** `SetVector` не ограничивается своим хранилищем.
Дальше она проходит по детям 0, 1, 2:

```asm
push 0 / mov ecx, ebp / call GetChild        ; ребёнок n
test ebx, ebx / jz skip                       ; нет ребёнка -> следующий
mov edx, [ebx] / lea eax,[esp+0x10] / push eax
mov ecx, ebx / call [edx+0x10]                ; child->GetChannelType()
add eax, 0x60                                 ; -> baseguid
repe cmpsd  (4 dword'а против 0x10002120)     ; == BE69CCC4-… , числовое семейство
jne skip
push <компонент> / mov ecx, ebx / call [eax+0x4c]   ; child->SetFloat(component)
```

Константа по `0x10002120` — `BE69CCC4-CFC1-4362-AC81-767D199BBFC3`, то есть **`Value`**. Значит:
запись в `Value Vector` **пробрасывается в числовые каналы, подключённые к его компонентам**, если
они подключены. Проверка семейства перед вызовом — та же дисциплина, что §3.2 предписывает нам.

Практическое следствие для скриптов: `vector_ch:set` — не локальное присваивание, как
`float_ch:set`, а запись с более широким радиусом. Там, где к компонентам подключены обычные
`Value`, это ровно то, чего хочет автор скрипта; там, где вычисляемые каналы, — запись переживёт до
следующего пересчёта. Задокументировано в `Docs/scripting/channels.md`.
...
### 2.2.3 Табличные расширения: `Aco_Array_Value` и `Aco_Array_Vector`

Снято затем, чтобы закрыть открытый вопрос ступеней B и C плана Ф5 (`lua-scripting.md`):
**переопределяют ли табличные типы сеттеры так, чтобы запись попадала в текущую строку, а не в
собственное скалярное поле канала?** Ответ — **да, оба**, но по дороге выяснилось четыре вещи,
каждая из которых меняет то, как этот сеттер можно выставлять наружу.

Типы найдены через `engine/channels.lst` (записи `CHTY` — имя типа, `C1FN` — файл):
`Array Value` → `DF5BF7F7-C204-4F6E-BDB8-666A53DFCC58.dll`,
`Array Vector` → `DD626E09-F497-4A34-9032-47AD4D2BCBD7.dll`.

#### Слоты

`??_7Aco_Array_Value@@6B@` (RVA `0x3130`), 36 слотов, наследник **`Aco_FloatChannel`**:

| Слот | Смещение | Метод | Чей |
|---|---|---|---|
| **17** | `+0x44` | `GetFloat` | **свой** (`0x12c0`) |
| 18 | `+0x48` | `GetOldFloat` | thunk → `Aco_FloatChannel` |
| **19** | **`+0x4c`** | **`SetFloat(float)`** | **свой** (`0x1350`), `ret 4` |
| 24–27 | `+0x60`… | `GetHighestValue` / `GetLowestValue` / `GetTotalValue` / `GetAverageValue` | свои |
| 28–35 | `+0x70`… | `Get/SetTableGUID`, `GetTableNum`, `Get/SetColumnGUID`, `GetColumnNum`, `Get/SetOldStyle` | свои |

`??_7Aco_Array_Vector@@6B@` (RVA `0x3128`), 28 слотов, наследник **`Aco_VectorChannel`**:

| Слот | Смещение | Метод | Чей |
|---|---|---|---|
| **17** | `+0x44` | `GetVector` | **свой** (`0x1a40`) |
| **18** | **`+0x48`** | **`SetVector(D3DXVECTOR3)`** | **свой** (`0x1240`), `ret 0xC` |
| 19 | `+0x4c` | `SetFloat(int, float) -> bool` | свой |
| 20–27 | `+0x50`… | `Get/SetTableGUID`, `GetTableNum`, `Get/SetColumnGUID`, `GetColumnNum`, `Get/SetOldStyle` | свои |

**Практический вывод номер один: нового ABI не нужно.** Пишущий слот у `Array Value` — 19
(`SetFloat(float)`, один dword, `ret 4`), у `Array Vector` — 18 (`SetVector`, три dword-а по
значению, `ret 0xC`). Это ровно те же сигнатуры и те же смещения, что у базовых семейств §2.2 и
§2.2.2, то есть `tw::lua::channels::set_float` и `set_vector` **уже байт в байт корректны для этих
типов** — менять в них нечего. Обратите внимание, что раскладка слотов при этом семейственная, а
не общая: у `Array Value` слот 18 занят `GetOldFloat`, у `Array Vector` — `SetVector`. Правило §3.2
(проверять тип до вызова) держится и здесь.

#### Как выбирается строка

Оба сеттера делают одно и то же и делают это **так же, как геттеры**:

```asm
push 0 / mov ecx, esi / call GetChild         ; ребёнок 0 — курсор
test eax, eax / je skip
mov edx,[eax] / mov ecx,eax / call [edx+0x44] ; cursor->GetFloat()   (слот 17)
call <ftol>                                   ; усечение к int
mov [esi+0xa0], eax                           ; номер строки в поле канала
skip:
...
push 0xc / push edi / push eax                ; size, &value, row
call [edx+0x58]                               ; connect->SetData(row, src, size)
```

То есть **номер строки берётся из значения ребёнка 0 в момент вызова** — не из аргумента и не из
какого-то отдельного «курсора записи». Это ровно та механика, вокруг которой уже построен
`read_array_vector`, поэтому существующий танец «сохранить курсор → выставить → операция →
безусловно вернуть» переносится на запись **дословно**. Поле с номером строки: `+0xa0` у
`Array Vector`, `+0x90` у `Array Value`.

#### Запись не мемоизирована — и это ловушка для пробника

Геттеры начинаются с `CheckRenderCount()` и при повторном вызове за кадр **выходят сразу**, отдавая
закэшированный скаляр из `+0x7c`. Сеттеры этой проверки **не делают вообще**. Отсюда две вещи:

- **для записи bust мемоизации не нужен** — она всегда доезжает (упрощение против чтения);
- **чтение сразу после записи в том же кадре ничего не проверяет.** Оба сеттера безусловно кладут
  значение и в собственное поле `+0x7c` тоже, а геттер при сработавшей мемоизации вернёт именно
  `+0x7c`. Значит зонд вида «записал → прочитал → совпало → значит записалось в таблицу» **всегда
  скажет «да»**, даже если табличная запись была молча пропущена. Проверять надо либо после bust-а
  мемоизации, либо чужими глазами (следующий кадр, другой потребитель таблицы).

Это тот же класс ошибки, что уничтожил первую версию `vecwrite.lua`: прибор, измеряющий сам себя.

**Уточнение, без которого этот абзац читается строже, чем следует.** Условие «проверять после bust-а»
`read_array` / `read_array_vector` выполняют **сами** — они инвалидируют мемо перед каждым чтением,
иначе обход таблицы возвращал бы строку 0 раз за разом. Плюс у большинства табличных колонок `CHIC=1`
(400 из 431), то есть они не мемоизируются вообще. Значит чтение **через `array:get` — настоящее
чтение таблицы**, и проверка записи им законна; ловушка относится к чтению той же колонки обычным
`float_ch`/`vector_ch`-хендлом, где bust-а нет.

Опасность, которая остаётся и от которой bust после записи, — не наша проверка, а **читатели игры**:
они ничего не инвалидируют и до конца кадра могут держать протухший скаляр.

#### Молчаливая деградация вместо ошибки

Оба сеттера возвращают `void`. Сигнала об отказе нет **никакого**, а отказаться они могут:

- `Aco_Array_Value::SetFloat` первым делом проверяет `[esi+0x88]` (флаг «таблица привязана», он же
  проверяется в геттере) и при нуле **пропускает всю табличную часть**, выполняя только
  `Aco_FloatChannel::SetFloat(v)` — то есть тихо превращается в запись скаляра;
- `Aco_Array_Vector::SetVector` спрашивает `connect->GetIfValid()`, при `false` зовёт
  `connect->Connect()` и спрашивает ещё раз; если и там `false` — табличная запись пропускается,
  скаляр в `+0x7c` уже лежит.

Плюс асимметрия защиты: `GetVector` проверяет указатель на connect-объект на `NULL`
(`test ecx,ecx / je`), а `SetVector` **разыменовывает его без проверки**. Путь записи в этом типе
менее защищён, чем путь чтения.

#### Границ нет, и это хуже, чем «нет границ»

Объект по `+0xa4` (`Array Vector`) / `+0xb0` (`Array Value`) — это `ArrayConnectItem`, чья
реализация лежит в DLL типа **`Array Unique`** (`2346A6DF-…`, vtable RVA `0x4138`, 24 слота):

| Слот | Смещение | Метод |
|---|---|---|
| 0 | `+0x00` | `GetIfValid -> bool` |
| 1 | `+0x04` | `Connect` |
| 2 | `+0x08` | `Disconnect` |
| 13 | `+0x34` | `GetRowCount -> int` |
| 14 | `+0x38` | `AddRow -> int` |
| 15/16 | `+0x3c`/`+0x40` | `InsertRow(int)` / `CreateRow(int)` |
| 17 | `+0x44` | `GetRowOrCreate(int) -> RowItem*` |
| **18** | **`+0x48`** | **`GetRow(int) -> RowItem*`** ← NULL для несуществующей, **без создания** |
| 19 | `+0x4c` | `RemoveRow(int)` |
| **20** | **`+0x50`** | **`GetData(int) -> void*`** ← этим читают геттеры |
| 21 | `+0x54` | `GetDataSize(int) -> int` |
| **22** | **`+0x58`** | **`SetData(int, void*, int)`** ← этим пишут сеттеры |
| 23 | `+0x5c` | `GetDataTimeStamp(int) -> DWORD` |

`ArrayConnectItem::SetData` (RVA `0x1f00`) целиком:

```
col = this[+0x2c];  if (!col) return;
r = col->GetRow(row);
if (!r) { col->CreateRow(row); r = col->GetRow(row); if (!r) return; }
r->SetData(src, size);
col->slot_0x60();          // отметка изменения / timestamp
```

**Индекс не проверяется, а несуществующая строка создаётся.** Это принципиально хуже, чем отсутствие
проверки: промах мимо диапазона не «ничего не делает» и не «портит соседнюю строку» — он
**удлиняет таблицу игры**. `GetRowCount()` после этого другой, и всё, что таблицу обходит, видит
лишнее. Ровно тот сорт поломки, о котором предупреждает §3.7 gameplay-журнала: не крашится, просто
дальше всё немного неправда.

Отсюда прямое требование к API: **`array:set` обязан отказывать на индексе, которого нет**, а не
передавать его в `SetData`. Дешёвый и точный способ уже есть — `ArrayConnectItem::GetRow(row)`
(слот 18) возвращает `NULL` для несуществующей строки и **ничего не создаёт**; `GetRowCount`
(слот 13) даёт границу явно. Цена — знание смещения connect-объекта в канале (`+0xa4` / `+0xb0`,
зависит от типа), то есть ещё две захардкоженные константы раскладки, которые надо охранять так же,
как `is_callable_as` охраняет слоты: проверкой, что указатель читаем, а адреса в его vtable
исполнимы.

Заодно из этой таблицы видно, что доступна не только запись ячейки: `AddRow` / `InsertRow` /
`RemoveRow` — полноценная поверхность управления таблицей. Наружу её выставлять **не следует**
(строка таблицы игры — это объект в её собственной модели, а не ячейка в массиве), но знать, что
она лежит в одной vtable рядом с `SetData`, полезно: промах по номеру слота попадёт именно туда.

#### Чем это снято

`Tools/CgrPy/`: `chtypes.py` (разбор `channels.lst` в «тип → dll»), `vtdump.py` (vtable
по экспорту `??_7…@@6B@` с разрешением имён по таблице экспортов и определением конца таблицы),
`imps.py` (импорты — по ним виден родитель типа), `findvt.py` / `findtab.py` (поиск
неэкспортированных vtable по форме: прогон указателей в `.text` плюс совпадение `ret imm16` у
конкретных слотов — так нашёлся `ArrayConnectItem`), `disasm.py` (capstone, с разрешением
`call [iat]` в имена импортов). Каталог `Temp/` в `.gitignore` — инструмент переживает сессию, но
не клон репозитория.
...
> **Поправка (`reversing-journal-boot.md` §1.2).** «Ровно один вызов на группу за кадр… реально — по
> числу живых групп» — неверно. `EngineControl::EngineLoop()` зовёт `CallStartChannel` **один раз за
> кадр, у стартовой группы**, чей индекс берётся из `EngineInterfaceExt::GetStartGroup()`; все
> остальные группы за кадр так не вызываются. Детур сюда поэтому стоит один переход за кадр, а не
> 161. И наоборот, «один вызов за кадр» **не гарантирован**: `CallStartChannel` экспортируется и
> импортируется каналом `Lua Script`, то есть его может дёрнуть игровой скрипт (§6.4 там же).
> Для покадровой зацепки надёжнее `EngineControl::EngineLoop` — она же даёт «до графа» и «после
> графа».
...
> **Уточнено `reversing-journal-boot.md` §1.2.** Отложенность — свойство **этого способа**, а не
> движка. `EngineControl::EngineLoop` (экспортируется, зовётся раз за кадр из главного цикла) отдаёт
> `EngineInterface*` прямо из `this` — через `EngineControl::GetEngineInterface()`, слот 5, — то есть
> **на первом же кадре и без единого клика**. Хук на `CallChannel` ради указателя не нужен вовсе;
> вместе с ним уходит и «факт 5», и необходимость в `DetourDetach` ниже. План замены —
> `lua-engine-fix-roadmap.md`, Ф1.
...
- ~~**Кто и как часто инкрементит глобальный tree count**~~ — закрыто,
  `reversing-journal-boot.md` §1.2: `EngineControl::EngineLoop()` зовёт
  `EngineInterfaceExt::IncreaseTreeCount()` **ровно один раз за кадр и безусловно**, даже если
  стартовой группы нет. `A3d_ChannelGroup::GetTreeCalculateCount` при этом по-прежнему сумма
  глобального счётчика и собственного счётчика группы (`+0x54`), RVA `0x9840`, экспортируется.
...
- ~~**Векторный ABI**~~ — закрыто, см. §2.2.2. Подозрение про `SetFloat(int, float)` подтвердилось
  ...
  **Перед любым `SetFloat` тип канала обязан быть проверен** — правило остаётся в силе, теперь с
  доказательством.
- ~~**Матричный ABI**~~ — закрыто, `reversing-journal-boot.md` §7.1. `Aco_MatrixChannel`: слот 17
  `GetMatrix` → `D3DXMATRIX` по значению, слот 18 `GetOldMatrix` (тот же адрес, COMDAT), слот 19
  `SetMatrix(D3DXMATRIX)` по значению; таблица кончается на слоте 20. Там же сняты слоты семейств
  `Aco_DX8_Texture`, `Aco_DX8_ObjectDataChannel` и `Aco_DX8_ObjectChannel`, и они резко усиливают
  правило «проверяй тип до вызова»: слот 17 у текстуры — `InvalidateDeviceObjects`, у `3D Object` —
  **`DrawSurfaces`**, а слот 19 у первых двух — **`Release`**.
...
- **Порядок вызова групп за кадр** по-прежнему не выписан (перешло из
  `reversing-journal-gameplay.md` §9), но вопрос переформулирован `reversing-journal-boot.md` §10:
  за кадр `CallStartChannel` получает **одна** группа, стартовая, а до остальных доходят по
  ссылкам — то есть спрашивать надо не «в каком порядке движок зовёт группы», а «в каком порядке
  стартовый канал `XX_StartHere` доходит до чужих». Отвечает на это детур на `CallChannel`
...
Разбор графа (гистограмма типов каналов, пересчёт переопределений `CallChannel`) — через
`Tools/CgrPy/`, см. §8 `reversing-journal-gameplay.md` и `Tools/CgrPy/README.md`.
