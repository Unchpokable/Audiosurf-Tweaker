...
Отдельно — **аудио**: спектр, громкость и позицию в песне игра считает сама каждый кадр и кладёт
в обычные каналы `sounds/VisMusic.cgr`, так что перехватывать BASS не нужно вовсе — §10.
...
`VectorOperator`). Но **палитра там не одна**, и это выяснилось дорогой ценой: запись в #1535
поменяла значение канала (скрипт, читающий его, увидел новый цвет), а блоки на трассе остались
прежними. Причина — три вложенные развилки перед палитрой:

```
FetchColorByID (#882)
└─ #1533  switch: (XX_gui::Color5.x + .y + .z) == 0 ?
   ├─ 0  цвета заданы  → #7157  switch: SpecialPurpose::isMechMode
   │                     ├─ 0 → #7072 switch: SpecialPurpose::PortalMode?
   │                     │      ├─ 0 → #440   → XX_gui::Color1..Color5   ← ОБЫЧНАЯ ИГРА
   │                     │      └─ 1 → #7074  → PlayerCar_Sword::PortalColor1..5
   │                     └─ 1 → #7159  (палитра мех-режима)
   └─ 1  цвета нулевые → #1535  → StartGroup #1542/#1538/#1534/#1530/#1526   ← ЗАПАСНАЯ
```

**#1535 — запасная ветка**, живая только когда `XX_gui::Color5` нулевой. В обычной игре палитру
даёт **#440, а он импортирует `XX_gui::Color1..Color5`**. Нумерация там обратная: `ColorID` 0
(фиолетовый) → `Color5`, `ColorID` 4 (красный) → `Color1` (#8060 в `XX_gui`). Дубль сохраняется —
случаи 4 и 5 оба ведут в `Color1`.

Вывод, который стоит держать в голове при любой работе с этим графом: **прочитать канал и увидеть
правдоподобное значение — не доказательство, что игра читает именно его.** У `Value Vector` нет
признака «мной пользуются»; отличить рабочую ветку от запасной можно только пройдя развилки вверх
или проверив селектор в рантайме.

**И вторая половина того же урока — почему запись в рабочую ветку тоже не сработала.** Компоненты
у двух палитр устроены по-разному:

```
StartGroup #1526 (запасная)          XX_gui::Color1 (рабочая)
  ├─ Value "default"   лист            ├─ Value "colorcomponent" ─ Envelope "Slider Value"
  ├─ Value "default"                   ├─ Value "colorcomponent" ─ Envelope
  └─ Value "default"                   └─ Value "colorcomponent" ─ Envelope
```

`SetVector` пишет на один уровень вниз — в компоненты. У #1526 это **листья**, и запись держится
(проверено на живой игре). У `Color1` за каждым компонентом стоит `Envelope` ползунка настроек, и
`Aco_FloatChannel::GetFloat` при следующем пересчёте затирает наше значение тем, что даёт ползунок.

Причём затирает **строго через кадр**, и это следствие `CheckRenderCount`, а не гонка:

```
кадр N     скрипт пишет      -> мемо горячий, движок не пересчитывает -> наше значение живо
кадр N+1   мемо сброшен      -> движок пересчитал из Envelope         -> наше значение стёрто
```

Замерено на живой игре: ровно **50 %**. То есть «оспариваемая» запись это не «обычно затирают», а
«затирают через раз», и любой читатель канала видит значение, скачущее между нашим и игровым.

Общее правило: **канал, в который что-то воткнуто, — производный; запись держат только листья.** Общее правило: **канал, в который что-то воткнуто, — производный; запись
держат только листья.**

Раскладка самой палитры (одинаковая у #1535, #440 и #7074 — меняются только каналы за случаями):
связь [0] — селектор (`ColorID`), связи [1..] — случаи по порядку.
...
#### Чем именно режим ужимает палитру

`Do_CharacterAndLeagueTrafficMods` (#904) вызывается из `Do_ResetStats` и переписывает
`Stats: TrafficPattern` **до** заезда. Условия и подстановки (все проверены по графу):

| условие | что делает | остаётся цветов |
|---|---|---|
| `Ninja?` \|\| `Freeride?` | `Do_NinjaMagic` (#403) переписывает **каждую** строку: `x := (x > NinjaColorCutoff) ? 3 : Puzzle::StoneColorID`. Порог — `ChannelSwitch#1836(LeagueID)` = 1/2/2, плюс `SpecialPurpose::GreyMode` (1 → −1, 2 → 999 «всё серое», 3 → −999 «серого нет»). У Freeride с `FreerideType != 2` подставляется просто `3` | **2**: слот жёлтого (3) и слот белого (стоун) |
| `ThinTraffic?` \|\| `Ninja?` | **прореживание** (см. ниже), у не-Ninja плюс `Do_TurnGreensBlue` (2 → 1) и `Do_TurnPurplesBlue` (#877, 0 → 1) | — |
| `LeagueID == 1` | `Do_LimitTo4ColorsForProMode` (#886) → `Do_TurnGreensBlue`: 2 → 1 | **4**: 0, 1, 3, 4 |
| `isMechMode` | `Do_TurnYellowRed` (3 → 4), `Do_TurnBlueGreen` (1 → 2) | — |
| `PortalMode? == 1` | 0 → 1, 3 → 2, 4 → 2 | — |

То есть у Casual на трассе три цвета, у Pro четыре, у Mono ровно два, и это **свойство паттерна**, а
не отдельный флаг режима. Внешнему UI, который показывает колонку на цвет, правильно брать набор
колонок из самого паттерна после обхода: тогда все шесть случаев и любые будущие покрываются одним
условием «в этом цвете ноль блоков», а таблица режимов — устаревает.

#### Прореживание: строка `(0, 0, 0)` — это не блок

Внутри того же цикла (`If` #666, условие `ThinTraffic? || Ninja?`) машинки, идущие слишком плотно,
**убираются с трассы обнулением всей строки**:

```
Do_NeuterThisCar (#680):  Stats.TrafficPattern[i] := Value Vector#684 = (0, 0, 0)

    Ninja?      :  x > 5  &&  (y - LastAllowedRing) < ChannelSwitch#932(LeagueID)  -> neuter
    иначе       :  x < 3  &&  (то же условие)                                      -> neuter
    в обоих     :  else -> Do_AllowThisCar (#681): LastAllowedRing := y
```

Строка остаётся в массиве, и её `x` равен нулю. Отсюда две вещи:

- **`Do_GetTrafficCounts` считает такие строки фиолетовыми** — его `ChannelSwitch #1248` смотрит
  только на `x` и кладёт 0 в `PurpleTotal`. То есть игровой `PurpleTotal` у Mono и у «прореженных»
  персонажей завышен на число выброшенных машинок. На бонусы это не влияет: жёлтый и красный
  прореживание не трогает ни в одной из веток, а Ninja и так исключён из цветовых бонусов.
- **У не-Ninja это маскируется случайно.** `Do_TurnPurplesBlue` (#877) идёт следующим шагом той же
  итерации и превращает свежий ноль в синий. У Ninja-ветки такого шага нет — поэтому фантомная
  фиолетовая колонка видна именно в Mono. (`Do_TurnBluesPurple` #781, который стоит после цикла, —
  пустой `ChannelCaller` без детей, он не делает ничего.)

Внешнему трекеру строку `(0, 0, 0)` надо пропускать, и проверять **все три компонента**, а не только
цвет: у настоящего фиолетового блока есть кольцо.
...
### 3.12 Skill Rating: число, которого игрок не видит

`Support/Achievements.cgr::Do_CalcSkillRating` (#1189, имя уникально), вызывается из
`Do_FinalizeAndStringEncodeExtendedRideStats` (#1118), который зовёт сам `Do_CalculateFinalStats`:

```
SkillRating (#1096) := MAX(1, ROUND( StatCollector::PointsWithGridBonus
                                     / StartGroup::GoldRequirement * 100
                                     * (StartGroup::LeagueID + 1) ))
```

Дальше он уезжает строкой в `sExtendedStats` на сервер и **нигде на экране не показывается**. Это
делает его опасным для стороннего HUD: сверить не с чем, и заниженное значение читается игроком не
как баг, а как «что-то я сегодня разучился играть».

Три слагаемых, каждое из которых выглядит как константа и ни одно ей не является:

- **`PointsWithGridBonus`** — §3.4. Все скейлеры берутся от **сырых** `Points`, не компаундятся:
  `Points * (1 + сумма скейлеров)`. Складывать их скриптом законно.
- **`GoldRequirement`** — не порог песни, а `TotalCarCount * {10, 30, 35}[LeagueID]` (§3.6). Читать
  канал.
- **`LeagueID`** — свойство **персонажа**, а не сложности: пишется в
  `Stage/XX_WindowState.cgr::Do_SetCharacter` (#307), пятнадцатью `Set Value` со значениями 0/1/2 по
  персонажу. Сложность — это `ChosenDifficulty`, она выбирает другой набор порогов
  (`Do_SetMedalRequirements` #407, `SetConstants::Medals: Easy/Normal/HardGold`) и в рейтинг не
  входит. Множитель — именно `LeagueID + 1`.

**Где в этом ошибся `puzzlepro_hud.lua` (исправлено в v1.2).** Формула и все три входа были верны;
неверной была композиция бонусов. `Do_FindAddBonusPoints` проверяет `NumTilesInGrid == 0` **в момент
подсчёта**, то есть бонус Clean Finish принадлежит игроку всё то время, пока решётка пуста, — это не
«оптимистичный сценарий». Скрипт же держал его только во второй, скобочной цифре, и главное число
занижало рейтинг на 20 % (`1 / 1.25`) ровно тогда, когда игрок держит чистую решётку — то есть почти
весь хороший puzzle-заезд и весь его конец.

Проверять такие вещи стоит именно так, как это вскрылось: не «сходится ли формула», а «сходятся ли
все её входы **и** порядок, в котором игра их складывает».
...
`Tools/CgrPy/` — Python-пакет под `uv`, Python 3.13:
...
| `cgr/pe.py` | минимальный PE-ридер: RVA↔offset, экспорты, импорты, vtable |
| `cgr/paths.py` | где лежит игра; единственное место, которое это знает |
| `rdy2use/` | готовые инструменты — `build_proj`, `render`, `meshes`, `vtdump`, `findvt`, `disasm`, `symbols`, `chtypes` |
| `examples/genall.py` | генерация всего набора артефактов |

Полный индекс с примерами и указанием, где какой инструмент пригодился, — `Tools/CgrPy/README.md`.
Запуск через `Tools/CgrPy/uvrun.bat`, зависимости ставит `uv` сам.
...
## 10. Аудио: что игра знает о играющей музыке

Раздел появился под конкретную задачу — сделать скайбокс реагирующим на музыку. Вывод короткий:
**ничего перехватывать не нужно**. Игра уже считает спектр, громкость и позицию в песне каждый
кадр, кладёт результат в обычные каналы графа, и всё это читается тем же способом, что и любая
другая статистика из §7.

### 10.1. Где это живёт

Звук у игры на **BASS**, и у Quest3D есть плагин-обёртка над ним: в `channels.lst` живут типы
`BASS`, `BASS_GetFFT256`, `BASS_GetLevels`, `BASS_GetSongPosition`, `BASS_GetState`,
`BASS_PlayFile`, `BASS_PreCalcSong` и ещё десяток. Единственный экземпляр канала `BASS` (ядро
библиотеки) — `sounds/VisMusic.cgr` #2; на него ссылаются **32 импорт-стаба** из 15 групп, то есть
это действительно один общий звуковой контекст на всю игру, а не по копии на сцену.

`sounds/VisMusic.cgr` (627 каналов) — и есть аудио-фронт игры. Её `Do` (#0) вызывается из
`XX_StartHere::DoMusic` (#10), а тот — из `Do_Gameplay` (#24) → `Do_MainGameMode` (#1624). Значит,
**во время заезда VisMusic отрабатывает каждый кадр**. Второй вызов, `XX_StartHere::Do_Bass`
(#433), висит на `MainMenu` — в меню данные тоже живые.

### 10.2. Что VisMusic делает за кадр

Декомпиляция `Do` (`cgr/decomp.py`), сокращённо:

```
if UseQuickTimeFFT? { QuickTime_GetFFT#406 }        // MP3 через iTunes
else if MinimalDetail? { BASS_GetFFT256#591 }       // урезанный путь
else { BASS_GetFFT256#6 }                           // обычный путь
...
Bass_FileCDplayTime := BASS_GetSongPosition(BASS)
RunningTime_Seconds := Bass_FileCDplayTime
SongPlayRemaining   := SongLength - Bass_FileCDplayTime
```

Два места стоят отдельного внимания.

**`TotalSpectrumHitThisFrame` уже нормирован** — поделён на `Highway::maxIntensity`, пик
интенсивности, найденный при предварительном анализе **этой** песни, и обрезан по 1. То есть это не
«сколько сейчас энергии в спектре», а «насколько сейчас громко относительно самого громкого места
песни», 0..1. Для шейдера это ровно то, что нужно: одинаковый диапазон на тихом акустическом треке
и на громком. Считать такое самому — значит заново решать задачу, которую игра уже решила лучше
(у неё есть весь трек целиком, у нас — только текущий кадр). У канала **179 потребителей** внутри
самой игры: камера, GUI, `Render_IndustrialTunnel`, `RoadSupports` — то есть игра сама на нём
строит визуальную реактивность.

**Отбрасывание `> 5`** в сумме — примитивный выброс-фильтр: полоса, вылетевшая за 5, в сумму не
идёт вообще (не клипуется, а обнуляется).

### 10.3. Инвентарь: что читаемо

Всё в группе `VisMusic`. Имена уникальны внутри группы — **кроме `SampleCount`** (#10 и #566;
к `BASS_GetFFT256` привязан #10, и брать его надо по индексу, иначе `GetChannel(name)` вернёт не
тот, см. `lua_channels.hxx` про неуникальность имён).

| Канал | # | Тип | Что это |
|---|---|---|---|
| `TotalSpectrumHitThisFrame` | 11 | Value | громкость 0..1, нормирована по пику песни. 179 потребителей |
| `New Table: SpectrumHit` | 7 | Array Value | спектр, `SampleCount` полос (12) по 1808.7 Гц каждая, курсор — `SpectrumIndex` #8. Точная модель — §10.8 |
| `SpectrumIndex` | 8 | Value | курсор к #7 |
| `SpectrumHit256` | 239 | Array Value | по разводке — полный спектр, 256 бинов, курсор `Index_SpectrumHit256` #238. **На деле всегда ноль**, см. §10.9 — не строить на нём ничего |
| `Index_SpectrumHit256` | 238 | Value | курсор к #239 |
| `SampleCount` | 10 | Value | 12 — число полос в #7 |
| `RunningTime_Seconds` | 149 | Value | позиция в песне, секунды. 24 потребителя |
| `SongLength` | 5 | Value | длина песни, секунды. 53 потребителя |
| `SongPlayRemaining` | 45 | Value | сколько осталось |
| `BeatHitThisFrame?` | 126 | Value | флаг бита от `Do_PrimitiveBeatDetection` |
| `CurrentBeatVal` / `PreviousBeatVal` | 108 / 107 | Value | внутренности того же детектора |
| `Bass_State` | 429 | Value | состояние потока |
| `MusicFilePath` | 23 | Text | путь к файлу |

Один `BASS_GetFFT256` (#6) заполняет **оба** спектра сразу: порт 4 — 12-полосный `SpectrumHit`
(режим нормализации 1), порт 8 — 256-биновый `SpectrumHit256` (режим 2, индекс 255).

Важная асимметрия: 12-полосный пишут **все три** ветки (BASS обычная, BASS `MinimalDetail`,
QuickTime), а 256-биновый — **только обычная ветка BASS** (#6). `MinimalDetail?` и `UseQuickTimeFFT?`
по умолчанию 0, так что обычно 256 бинов есть; но полагаться на них как на всегда живые нельзя, а
на 12 полос — можно.

### 10.4. Грабли

**Деление на ноль.** `TotalSpectrumHitThisFrame = MIN(1, sum / Highway::maxIntensity)`. `Highway`
загружена только во время заезда, а `maxIntensity` до предварительного анализа песни равен нулю.
`MIN(1, x/0)` даёт либо 1, либо NaN (при `0/0`) — и NaN, попавший в константу шейдера, красит небо
в чёрное или в мусор без единого сообщения об ошибке. **Любой потребитель обязан санировать
значение** (`isfinite`, затем clamp 0..1), а не доверять тому, что имя канала обещает диапазон.

**Мемоизация массивов — здесь её нет, и проверять это надо руками.** У #7 и #239 `CHIC = 1`
(`ingoreTreeCountState_`), то есть колонки **не** мемоизируются и читаются сколько угодно раз за
кадр — иначе собственный `ForLoop` игры не смог бы их обойти. Ручной сброс `calculatedAt_` в
`lua_channels::read_array` для них не нужен (он и не сработает: код смотрит на реальный байт
объекта и на `CHIC = 1` ничего не трогает), но и не мешает.

Ловушка не в самом флаге, а в том, как его проверять: `cgr/graph.py` перечисляет `CHIC` в `HEADER`
и **выбрасывает** его, так что в `channel.chunks` его нет никогда, и любой тест вида
`any(t == "CHIC" ...)` отвечает «отсутствует» для всех 431 `Array Value` проекта разом — что
выглядит как осмысленный результат и им не является. Значение достаётся только повторным проходом
по сырому потоку чанков: `qchic.py`. Он же воспроизводит цифру из §9 (400 из 431 с `CHIC = 1`),
и именно это совпадение показывает, что проход правильный.

**Цена чтения.** Одна ячейка массива — сохранение курсора, запись, чтение, восстановление: четыре
виртуальных вызова. 12 полос — ничто; 256 бинов каждый кадр — уже заметная величина, и её надо
мерить, а не прикидывать.

### 10.5. Как это читать

Ничего нового не требуется — `lua_channels` покрывает всё:

```lua
local level    = tw.float_ch("VisMusic", "TotalSpectrumHitThisFrame")
local spectrum = tw.array("VisMusic", "New Table: SpectrumHit", "SpectrumIndex")
```

Все нужные каналы — `Value` и `Array Value`, то есть `GetFloat` на них это загрузка поля (плюс один
вложенный вызов курсора у массива), а не рекурсивный обход графа. Читать их с рендер-потока внутри
кадра безопасно по той же причине, по которой это уже делает `lua_host::draw_frame` из `EndScene`.

Зонд — `TweakerPlugin/assets/scripts/music.lua`: рисует все значения, их минимумы/максимумы за
сессию, 12-полосный спектр столбиками и собственную цену чтения в миллисекундах. Диапазоны полос
в документации не выписаны намеренно — их надо снять с реальной песни, а не унаследовать из
предположения.

### 10.6. Что показал зонд в живой игре

Первый прогон `music.lua` на реальных треках (2026-09-05):

- **Цена чтения — 0.014 мс** и редко выше. 12 полос каждый кадр плюс 256 бинов каждый четвёртый.
  То есть чтение графа в этом объёме бесплатно, и вопрос «сколько можно себе позволить» закрыт:
  можно всё.
- **`TotalSpectrumHitThisFrame` работает.** На треке с ударными подсветка попадает в бочку.
- **12-полосный спектр почти бесполезен.** На вокально-струнном материале (сопрано, скрипки,
  виолончели) реагирует полоса 0, слабее 1 и 2, остальные — только на тарелки и шорохи. На плотном
  метале середина оживает, но по характеру отклика похоже, что она ловит не инструменты, а
  высокочастотные искажения.

Такая картина — узнаваемая подпись **линейного** деления: если 256 бинов FFT (по 86.13 Гц каждый
при 44.1 кГц) нарезать на 12 равных кусков, полоса 0 накрывает 0–1837 Гц, то есть основной тон
почти всего, что играет, а полосы 3 и выше начинаются с 5.5 кГц, где из музыки живут одни тарелки.
Это ровно то, что наблюдается.

Но **на слух это не доказывается**, и на музыке — тоже: у любого реального трека энергия есть везде
сразу, поэтому «полоса 0 реагирует на всё» одинаково согласуется и с «полоса 0 — это 0–1.8 кГц», и
с «игра просто даёт ей больший вес».

### 10.7. Как это измерить точно

`Tools/SpectrumCalibration/` — ступенчатый тон: 61 чистая синусоида от 20 Гц до 20 480 Гц, по одной
на шестую октавы, каждая держится 0.75 с при −6 dBFS, между ними тишина. Когда играет ровно одна
частота, то, что засветилось, **и есть** её полоса.

Ступени, а не свип: свип размазывается по бинам на ширину окна FFT, и читатель на 60 кадрах в
секунду не знает, где свип был в момент выборки. Ступень постоянна 0.75 с — все кадры внутри неё
видят одно и то же, и расхождение между кадрами означает реальный эффект, а не промах по времени.

Читатель — `assets/scripts/spectrum_calibration.lua`. Считает по `RunningTime_Seconds`, в какой
ступени находится, накапливает отклик всех 12 полос и пиковый бин 256-массива и печатает вывод:
полосу пропускания каждой полосы по уровню половины её собственного пика и линейную регрессию
«номер бина от частоты». Если 1/наклон окажется около 86 Гц — широкий спектр линеен, как и должен
быть у необработанного FFT.

Обе половины проверяют себя, и обе проверки уже поймали настоящее:

- Генератор читает свой вывод обратно и делает FFT каждой ступени. Так нашлось, что **Vorbis на
  качестве по умолчанию срезает верх** и ступень 20 480 Гц возвращается тишиной — в игре это
  прочиталось бы как «у игры нет полосы наверху». Поэтому WAV: `.WAV` в `Do_SeeIfItIsWMA` идёт по
  тому же пути BASS (`SongFileType = 1`), что и `.OGG`, так что кодек не давал ничего.
- Читатель прогоняется против симулированной игры **дважды** — с линейными полосами и с
  логарифмическими. Он обязан выдать два разных ответа; совпадение означало бы, что он повторяет
  собственное допущение, а не измеряет.

Ограничение, о котором стоит помнить при чтении результата: бин шириной 86 Гц не разрешает низ.
Всё, что ниже ~80 Гц, живёт в бине 0 целиком — то есть у самой игры **нет** данных, по которым
можно было бы отличить 30 Гц от 70 Гц, и никакая обработка с нашей стороны их не создаст.

### 10.8. Результат калибровки: полосы линейны, и вот их точная модель

Прогон ступенчатого тона в живой игре (2026-09-05). Читатель записал все 61 ступень и выдал:

| полоса | измеренная полоса пропускания | пик |
|---|---|---|
| 0 | 20 Гц … 1.81 кГц | 1.14 кГц (0.064) |
| 1 | 2.03 … 3.62 кГц | 2.56 кГц (0.064) |
| 2 | 4.06 … 5.12 кГц | 4.06 кГц (0.062) |
| 3 | 5.75 … 7.24 кГц | 5.75 кГц (0.062) |
| 4 | 8.13 кГц | 8.13 кГц (0.061) |
| 5 | 9.12 … 10.24 кГц | 10.24 кГц (0.059) |
| 6 | 11.49 кГц | 11.49 кГц (0.054) |
| 7 | 12.90 … 14.48 кГц | 12.90 кГц (0.060) |
| 8 | 14.48 … 16.25 кГц | 16.25 кГц (0.049) |
| 9 | 16.25 кГц | 16.25 кГц (0.017) |
| 10 | 18.25 кГц | 18.25 кГц (0.057) |
| 11 | 20.48 кГц | 20.48 кГц (0.063) |

**Деление линейное.** Проверялись две модели: «12 равных долей от 0 до 22050 Гц» (по 1837.5 Гц) и
«256 бинов, нарезанных по 21 бину на полосу» (256 // 12 = 21, то есть 1808.7 Гц, а верхние четыре
бина не используются). Вторая укладывает 11 пиков из 12 в свои полосы против 10 у первой, и
решающая ступень — 18.25 кГц: у равных долей она попадает в полосу 9, у 21-бинной — в полосу 10, а
измерение говорит «полоса 10».
...
бины 252..255 не входят ни в одну полосу
```

Полоса 9 — единственный промах и единственное низкое значение (0.017): внутри её диапазона
16279…18088 Гц **нет ни одной ступени** калибровочной сетки (соседние — 16255 и 18246 Гц), так что
0.017 это утечка спектра от соседа. Дефект сетки, а не игры; ровный отклик 0.049…0.064 у всех
остальных как раз показывает, что режим нормализации 1 отдаёт плоскую характеристику по частоте.

**Что это значит на практике.** Полоса 0 — это 0…1.8 кГц, то есть основные тоны вообще всего:
бас, вокал, струнные, гитары. Полосы 1 и 2 — 1.8…5.4 кГц, обертоны и «презенс». Полосы 3…11 — всё
выше 5.4 кГц, где из музыки живут тарелки, шорохи и искажения. Наблюдение «реагирует первая, немного
вторая и третья» — это не странность игрового анализа, а прямое следствие линейного деления.

Значит, полезной информации в 12 полосах ровно четыре числа: **низ (0), середина (1), верх (2) и
воздух (сумма 3…11)**. И это надо считать потолком, а не поводом для разочарования: 86-герцовый бин
не разрешает низ в принципе — всё, что ниже ~86 Гц, лежит в бине 0 целиком, и никакая обработка с
нашей стороны разницы между 30 и 70 Гц не создаст, потому что её нет в данных.

### 10.9. `SpectrumHit256` читается и всегда ноль

Тот же прогон: канал **резолвится** (иначе читатель написал бы «nothing readable»), но ни одна из
256 ячеек ни разу не была отлична от нуля. Механизм чтения при этом заведомо исправен — 12-полосный
массив читается тем же `read_array` с тем же курсором, и вся калибровка выше на нём и построена.

Что проверено и **не** объясняет:

- **Ветка FFT.** `Render_IndustrialTunnel::MinimalDetail?` и `StartGroup::UseQuickTimeFFT?` обе
  равны 0 и **не имеют ни одного писателя во всём проекте** — ни внутри своей группы, ни через
  импорт-стабы. То есть игра всегда идёт по ветке `BASS_GetFFT256#6`, у которой порты 5–8 (256
  бинов) разведены.
- **Курсор.** Порт 0 канала #239 — это #238 `Index_SpectrumHit256`, ровно тот, что мы двигаем.
- **Мемоизация.** `CHIC = 1` у обоих массивов, они не мемоизируются.

Статический разбор упёрся: `87C266FD-5A43-419D-B402-44E1B0FFBE8F.dll` (тип `BASS_GetFFT256`,
14 КБ) **не импортирует `bass.dll` вообще** — данные он берёт через дочерний канал `BASS`, — и
`A3d_Channel::GetChild` он тоже не вызывает ни разу (импортирует, но не зовёт; проверено разбором
всех 2092 инструкций `.text` с разрешением импорт-танков). Значит, к детям он обращается напрямую по
раскладке объекта, и чтобы узнать, трогает ли он порты 5–8, нужен полноценный разбор функции
`Aco_BASS_GetFFT256::CallChannel`. Отдельная работа, и на текущую задачу не влияет.

**Рабочий вывод: 256-биновый спектр считать недоступным.** Строить на нём нельзя, а собственная
логарифмическая нарезка, которая на нём планировалась, отменяется — нарезать нечего.

Из этого следует правило для API шейдера: отдавать наружу **музыкальные величины** (низ, середина,
