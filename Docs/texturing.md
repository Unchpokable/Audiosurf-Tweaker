...
* Third group is a particles (particles1.png, particles2\3.jpg)
    ...
    * **About colouring particles.** The shipped atlases are greyscale, and that is not a style choice - the game multiplies every particle by a colour it picks itself (the colour of the block you collected in puzzle modes, the track colour in mono). Any colour you put in the atlas gets multiplied by that one, so a colourful atlas comes out as mud rather than as your palette. This is the same warning as for the hit textures above, and for particles it used to be absolute.
    * It is no longer absolute. The Tweaker ships a Lua script, `particles.lua`, that switches that tinting off - enable it in the overlay's **Scripts** tab. With it on, particles are drawn exactly as they are in your file, in both puzzle and mono modes. Two things follow: **the stock greyscale atlases will look washed out** while it is on (that is the tinting being gone, not a bug), and **you can finally ship a coloured particle atlas** and see your own colours on screen.
    * So there are two kinds of particle atlas now, and they are not interchangeable: a greyscale one for players who run the game as-is, and a coloured one that assumes the script. If you ship a coloured one, say so in your pack's description - otherwise it will look wrong to everyone who has not enabled the script.
