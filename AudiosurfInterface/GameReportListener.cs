...
{
    /// <summary>
    /// Typed view over AudiosurfHandle.MessageResieved's raw "asreport" content strings
    /// (e.g. "songcomplete 142798", "nowplayingartistname nine inch nails") - splits on the first
    /// space and raises a strongly-typed event per report kind, so callers don't each hand-parse the
    /// same prefixes. The three "nowplaying..." reports always arrive as three separate consecutive
    /// broadcasts for one song start (see GameProtocol_FromDylan.md) - buffered here and raised as a
    /// single NowPlaying event once all three have been seen.
    /// </summary>
